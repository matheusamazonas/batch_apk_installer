...
## Example
Let's take a look at an example setup and use of Batch APK Installer. Take the following config file with three platforms and two packages:
```toml
...
With the following APKs in `/Users/matheus/Desktop/my_builds`:
...
An example run of `batch_apk_installer my_builds -u` could output:
